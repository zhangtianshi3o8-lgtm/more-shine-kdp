#!/usr/bin/env python3
"""
Golf Log Book — KDP Full Wrap Cover Generator
...
Trim: 6" x 9"
Pages: 101 (cream paper)
Spine: 101 x 0.0025 = 0.2525"
Bleed: 0.125" all outer edges
Full cover: 12.5025 x 9.25 in
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "golf_log_book_cover_V1.0.html")

# KDP cover specs
TRIM_W = 6.0          # inches
TRIM_H = 9.0
PAGES = 101
SPINE = PAGES * 0.0025   # cream paper = 0.2525"
BLEED = 0.125
COVER_W = TRIM_W * 2 + SPINE + BLEED * 2   # 12.5025"
COVER_H = TRIM_H + BLEED * 2               # 9.25"

# Colors — Golf Course / Deep Green theme (quiet luxury aesthetic)
C_CHARCOAL = "#0D3B20"   # near-black golf green
C_DARK     = "#0D3B20"   # dark golf green
C_BROWN    = "#2A2520"   # dark earth
C_GREEN_D  = "#0D3B20"   # dark golf green
C_GREEN    = "#1A5C38"   # golf green
C_GREEN_L  = "#2d7a4f"   # lighter green
...
.front-cover {{
  width: {TRIM_W + BLEED:.4f}in;
  height: {COVER_H:.4f}in;
  background: linear-gradient(165deg, {C_CHARCOAL} 0%, {C_DARK} 25%, {C_GREEN_D} 55%, {C_DARK} 85%, {C_CHARCOAL} 100%);
  position: relative;
  overflow: hidden;
  display: flex;
  flex-direction: column;
...
<head>
  ...
  <title>Golf Log Book — Cover</title>
  {CSS}
</head>
<body>
<div class="cover-wrap">
  ...
  <div class="back-cover">
    <div class="back-text">
      <div class="blurb">
        <strong>Every round tells a story.</strong>
        From the first drive to the final putt, this journal
        helps you capture every shot, every score, and every
        great moment on the course.
      </div>
      ...
      <ul class="back-features">
        <li>40 rounds with full 18-hole scorecards</li>
        <li>Stat tracker for fairways, greens, and putts</li>
        <li>Season goals and handicap tracking</li>
        <li>Club distance reference</li>
        <li>My courses log</li>
        <li>Golf terminology and game formats guide</li>
        <li>Season calendar and stat summary</li>
  ...
  <div class="spine">
    ...
    <div class="spine-text">Golf Log Book</div>
  </div>

  <!-- ============ FRONT COVER ============ -->
  <div class="front-cover">

    <!-- Golf flag & hole illustration (SVG line art) -->
    <div class="glass-wrap">
      <svg viewBox="0 0 120 170" width="120" height="170" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Flag (triangle) -->
        <path d="M 60 10 L 102 26 L 60 42 Z"
              stroke="rgba(250,246,240,0.55)" stroke-width="1.8" fill="rgba(196,160,74,0.18)" stroke-linejoin="round"/>

        <!-- Pole finial (small circle at top) -->
        <circle cx="60" cy="8" r="2.5"
                stroke="rgba(250,246,240,0.55)" stroke-width="1.8" fill="none"/>

        <!-- Hole (ellipse at base) -->
        <ellipse cx="60" cy="135" rx="28" ry="6"
                 stroke="rgba(250,246,240,0.55)" stroke-width="1.8" fill="rgba(0,0,0,0.35)"/>

        <!-- Ground shadow line -->
        <ellipse cx="60" cy="142" rx="34" ry="3"
    ...
    <div class="title-block">
      ...
      <div class="features">
        <span class="feature-badge">40 Rounds</span>
        <span class="feature-badge">Scorecards</span>
        <span class="feature-badge">Stat Tracker</span>
        <span class="feature-badge">Handicap</span>
      </div>
      <div class="tagline">Track Every Round</div>
    </div>
