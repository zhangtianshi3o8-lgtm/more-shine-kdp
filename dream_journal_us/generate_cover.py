#!/usr/bin/env python3
"""
Dream Journal -- KDP Full Wrap Cover Generator
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "dream_journal_us_cover_V1.0.html")
...
# Colors -- Dark charcoal + deep lavender + gold
C_DARK     = "#161616"
C_DARK_M   = "#2A2530"
C_LAVENDER = "#8B7D9B"
C_LAV_D    = "#6B5D7B"
...
.back-cover::before {{
  ...
  background-image:
    radial-gradient(ellipse 24px 14px at 15% 25%, {C_LAVENDER}, transparent),
    radial-gradient(ellipse 22px 13px at 80% 15%, {C_GOLD}, transparent),
    radial-gradient(ellipse 26px 15px at 70% 70%, {C_LAVENDER}, transparent),
...
.back-cover::after {{
  ...
  border-radius: 50%; background: rgba(139, 125, 155, 0.08);
}}
...
.back-features li::before {{ content: ''; position: absolute; left: 0; top: 5px; width: 5px; height: 5px; background: {C_LAVENDER}; border-radius: 50%; }}
...
.spine::before {{ content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; opacity: 0.03; background-image: radial-gradient(ellipse 10px 6px at 50% 20%, {C_LAVENDER}, transparent), radial-gradient(ellipse 10px 6px at 50% 50%, {C_GOLD}, transparent), radial-gradient(ellipse 10px 6px at 50% 80%, {C_LAVENDER}, transparent); }}
...
.front-cover::before {{
  ...
  background-image:
    radial-gradient(ellipse 40px 24px at 15% 25%, {C_LAVENDER}, transparent),
    radial-gradient(ellipse 34px 20px at 80% 15%, {C_GOLD}, transparent),
    radial-gradient(ellipse 38px 22px at 70% 70%, {C_LAVENDER}, transparent),
    radial-gradient(ellipse 28px 18px at 25% 80%, {C_GOLD}, transparent),
    radial-gradient(ellipse 24px 15px at 50% 50%, {C_LAVENDER}, transparent);
}}
...
.accent-bar {{ width: 120px; height: 2.5px; background: {C_LAVENDER}; margin: 16px auto; }}
...
.feature-badge {{ background: rgba(255,255,255,0.08); border: 1px solid rgba(139,125,155,0.4); color: {C_GOLD}; font-size: 7.5pt; font-weight: 700; letter-spacing: 0.5pt; padding: 4px 10px; border-radius: 3px; text-transform: uppercase; }}
...
<head>
  ...
  <title>Dream Journal -- Cover</title>
  {CSS}
</head>
<body>
<div class="cover-wrap">
  ...
  <div class="back-cover">
    <div class="back-text">
      <div class="blurb">
        <strong>Every dream is a message from within.</strong>
        Whether you are exploring your subconscious, practicing lucid
        dreaming, or simply curious about the strange world that unfolds
        each night, this journal helps you capture, interpret, and learn
        from your dreams.
      </div>
      ...
      <ul class="back-features">
        <li>25 two-page dream record spreads</li>
        <li>Dream symbols reference guide</li>
        <li>Lucidity level and clarity tracking</li>
        <li>Weekly reviews every 5 dreams</li>
        <li>Lucid dreaming progress tracker</li>
        <li>Monthly summary and pattern spotting</li>
        <li>Large 6" x 9" format for detailed writing</li>
      </ul>
    </div>
  ...
  <div class="spine">
    ...
    <div class="spine-text">Dream Journal</div>
  </div>

  <div class="front-cover">
    <div class="icon-wrap">
      <svg viewBox="0 0 110 110" width="110" height="110" xmlns="http://www.w3.org/2000/svg">
        <!-- Crescent moon + stars (minimal line art) -->
        <g transform="translate(55,55)">
          <!-- Crescent moon -->
          <path d="M -6,-34 A 34,34 0 1,0 -6,34 A 25,25 0 1,1 -6,-34 Z"
                fill="none" stroke="{C_LAVENDER}" stroke-width="2.5" stroke-linejoin="round"/>
          <!-- Main star -->
          <path d="M 22,-16 L 25,-10 L 31,-9 L 26,-4 L 28,2 L 22,-1 L 16,2 L 18,-4 L 13,-9 L 19,-10 Z"
                fill="none" stroke="{C_GOLD}" stroke-width="1.5" stroke-linejoin="round"/>
          <!-- Small accent stars -->
          <circle cx="32" cy="14" r="1.5" fill="{C_GOLD}" opacity="0.7"/>
          <circle cx="-26" cy="-20" r="1.2" fill="{C_LAVENDER}" opacity="0.5"/>
          <circle cx="18" cy="26" r="1" fill="{C_LAVENDER}" opacity="0.4"/>
          <circle cx="-18" cy="24" r="0.8" fill="{C_GOLD}" opacity="0.3"/>
        </g>
      </svg>
    </div>
    <div class="title-block">
      <div class="main-title">Dream<br>Journal</div>
      <div class="accent-bar"></div>
      <div class="subtitle">Capture the Journey of<br>Your Subconscious Mind</div>
      <div class="features">
        <span class="feature-badge">25 Dream Logs</span>
        <span class="feature-badge">Symbol Guide</span>
        <span class="feature-badge">Lucid Tracker</span>
        <span class="feature-badge">Weekly Review</span>
      </div>
      <div class="tagline">For Dreamers & Seekers</div>
    </div>
