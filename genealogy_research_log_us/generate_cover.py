#!/usr/bin/env python3
"""
Genealogy Research Log Book -- KDP Full Wrap Cover Generator
Zero-dependency (Python stdlib only).
Trim: 6" x 9"
Pages: 66 (cream paper)
Spine: 66 x 0.0025 = 0.1650"
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "genealogy_research_log_us_cover_V1.0.html")
...
# Colors -- Deep plum / antique gold / cream
C_DARK     = "#1A1520"
C_DARK_M   = "#2A1F30"
C_PLUM     = "#5A3E5C"
C_BURGUNDY = "#7A4A4A"
...
.back-cover::before {{
  ...
  background-image:
    radial-gradient(ellipse 24px 14px at 15% 25%, {C_PLUM}, transparent),
    radial-gradient(ellipse 22px 13px at 80% 15%, {C_GOLD}, transparent),
    radial-gradient(ellipse 26px 15px at 70% 70%, {C_PLUM}, transparent),
...
.back-cover::after {{
  ...
  border-radius: 50%; background: rgba(122, 74, 74, 0.08);
}}
...
.back-features li::before {{ content: ''; position: absolute; left: 0; top: 5px; width: 5px; height: 5px; background: {C_PLUM}; border-radius: 50%; }}
...
.spine::before {{ content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; opacity: 0.03; background-image: radial-gradient(ellipse 10px 6px at 50% 20%, {C_PLUM}, transparent), radial-gradient(ellipse 10px 6px at 50% 50%, {C_GOLD}, transparent), radial-gradient(ellipse 10px 6px at 50% 80%, {C_PLUM}, transparent); }}
...
.front-cover::before {{
  ...
  background-image:
    radial-gradient(ellipse 40px 24px at 15% 25%, {C_PLUM}, transparent),
    radial-gradient(ellipse 34px 20px at 80% 15%, {C_GOLD}, transparent),
    radial-gradient(ellipse 38px 22px at 70% 70%, {C_PLUM}, transparent),
    radial-gradient(ellipse 28px 18px at 25% 80%, {C_GOLD}, transparent),
    radial-gradient(ellipse 24px 15px at 50% 50%, {C_PLUM}, transparent);
}}
...
.accent-bar {{ width: 120px; height: 2.5px; background: {C_PLUM}; margin: 16px auto; }}
...
.feature-badge {{ background: rgba(255,255,255,0.08); border: 1px solid rgba(90,62,92,0.4); color: {C_GOLD}; font-size: 7.5pt; font-weight: 700; letter-spacing: 0.5pt; padding: 4px 10px; border-radius: 3px; text-transform: uppercase; }}
...
<head>
  ...
  <title>Genealogy Research Log Book -- Cover</title>
  {CSS}
</head>
<body>
<div class="cover-wrap">
  ...
  <div class="back-cover">
    <div class="back-text">
      <div class="blurb">
        <strong>Every family has a story worth preserving.</strong>
        This research log helps you trace your ancestors systematically --
        vital records, census data, military service, immigration, and the
        stories that connect generations. A permanent record of your
      ...
      <ul class="back-features">
        <li>25 two-page ancestor profile spreads</li>
        <li>Family tree index with 14 entries</li>
        <li>Census tracker pages for organizing decades</li>
        <li>Research source log per ancestor</li>
        <li>Space for stories and family memories</li>
        <li>Surname and region tracking pages</li>
        <li>Large 6" x 9" format for easy writing</li>
      </ul>
    </div>
  ...
  <div class="spine">
    ...
    <div class="spine-text">Genealogy Research Log</div>
  </div>
    ...
    <div class="icon-wrap">
      <svg viewBox="0 0 120 120" width="120" height="120" xmlns="http://www.w3.org/2000/svg">
        ...
        <g transform="translate(60,60)">
          <!-- Trunk -->
          <rect x="-3" y="15" width="6" height="35" rx="1" fill="{C_PLUM}" opacity="0.4"/>
          <!-- Main branches -->
          <line x1="0" y1="15" x2="0" y2="-35" stroke="{C_PLUM}" stroke-width="2.5"/>
          <line x1="0" y1="-5" x2="-22" y2="-25" stroke="{C_PLUM}" stroke-width="2"/>
          <line x1="0" y1="-5" x2="22" y2="-25" stroke="{C_PLUM}" stroke-width="2"/>
          <line x1="0" y1="-18" x2="-16" y2="-38" stroke="{C_PLUM}" stroke-width="1.5"/>
          <line x1="0" y1="-18" x2="16" y2="-38" stroke="{C_PLUM}" stroke-width="1.5"/>
          <!-- Nodes -->
          <circle cx="0" cy="-35" r="6" fill="{C_GOLD}" opacity="0.6"/>
          <circle cx="-22" cy="-25" r="5" fill="{C_PLUM}" opacity="0.6"/>
          <circle cx="22" cy="-25" r="5" fill="{C_PLUM}" opacity="0.6"/>
          <circle cx="-16" cy="-38" r="3.5" fill="{C_PLUM}" opacity="0.5"/>
          <circle cx="16" cy="-38" r="3.5" fill="{C_PLUM}" opacity="0.5"/>
          <circle cx="0" cy="15" r="5" fill="{C_PLUM}" opacity="0.7"/>
        </g>
      </svg>
    </div>
    <div class="title-block">
      <div class="main-title">Genealogy<br>Research Log</div>
      <div class="accent-bar"></div>
      <div class="subtitle">Trace Every Ancestor,<br>Every Record, Every Story</div>
      <div class="features">
        <span class="feature-badge">25 Ancestor Profiles</span>
        <span class="feature-badge">Census Tracker</span>
        <span class="feature-badge">Source Log</span>
        <span class="feature-badge">Family Index</span>
      </div>
      <div class="tagline">For Family Historians</div>
    </div>
